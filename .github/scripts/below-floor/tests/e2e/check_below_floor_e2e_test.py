"""End-to-end tests for the below-floor check: the real entry point, no mocks.
...
SCRIPT = Path(__file__).resolve().parents[2] / "check_below_floor.py"
...
def test_e2e_red_check_passes_when_the_command_fails(capsys):
    ...
    assert run(["check_below_floor.py", "fail", "false"]) == 0
