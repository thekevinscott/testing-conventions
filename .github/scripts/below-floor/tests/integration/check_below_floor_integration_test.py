...
import check_below_floor as m  # noqa: E402
...
def test_main_passes_when_every_check_meets_its_expectation(command, capsys):
    ...
    assert m.main(["check_below_floor.py"]) == 0
    out = capsys.readouterr().out
    assert "::error::" not in out
    assert "[below-floor suite trips the coverage gate] ok" in out
    assert command.call_count == 1


def test_main_fails_when_check_0_is_violated(command, capsys):
    command.side_effect = _returns(0)
    assert m.main(["check_below_floor.py"]) == 1
    assert "::error::[below-floor suite trips the coverage gate]" in capsys.readouterr().out


def test_main_runs_the_hardcoded_commands(command):
    command.side_effect = _returns(1)
    m.main(["check_below_floor.py"])
