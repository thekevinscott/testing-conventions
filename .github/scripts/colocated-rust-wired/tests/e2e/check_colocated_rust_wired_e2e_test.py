"""End-to-end test for check_colocated_rust_wired: runs the real `__main__` entry via runpy.
...
SCRIPT = Path(__file__).resolve().parents[2] / "check_colocated_rust_wired.py"

BROKEN = "language: ${{ fromJSON(needs.detect.outputs.languages) }}\n"
...
def test_e2e_passes_on_a_wired_workflow(tmp_path, capsys):
    ...
    wf.write_text("language: ${{ fromJSON(needs.detect.outputs.colocated_test_languages) }}\n")
