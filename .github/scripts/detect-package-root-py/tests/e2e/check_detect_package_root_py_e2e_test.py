...
def test_e2e_fails_on_too_many_arguments(capsys):
    assert run(["check_detect_package_root_py.py", *GOOD_ARGS, "surplus"]) == 1
    assert "::error::usage:" in capsys.readouterr().out
...
def test_e2e_guard_does_not_run_main_for_a_lesser_module_name(capsys):
    ...
    assert run(["check_detect_package_root_py.py", *GOOD_ARGS], run_name="AAAAAA") == 0
...
def test_e2e_guard_runs_main_for_a_distinct_main_object(capsys):
    ...
    assert run(["check_detect_package_root_py.py", *GOOD_ARGS], run_name=distinct) == 0
    assert "package_root=.github/selftest/monorepo/packages/py" in capsys.readouterr().out
