...
both the input and command, one missing them — asserting exit code and printed line each way. The
two guard tests pin the `__name__ == "__main__"` comparison so mutating it (to `is` or `<=`) is
caught.
"""
import runpy
import sys
