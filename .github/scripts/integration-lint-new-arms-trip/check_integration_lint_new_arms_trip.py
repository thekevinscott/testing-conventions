#!/usr/bin/env python3
"""Assert the `integration-lint-new-arms-trip` self-test job's published-CLI exit codes (epic #302, #309).

Repo-only: this module exists *only in this repository*. It is NOT shipped in the published
`testing-conventions` tool; it backs the `integration-lint-new-arms-trip` job in
`.github/workflows/testing-conventions-selftest.yml`, which
drives the published `integration lint` command over the Rust and TypeScript arms — a
violating crate and suite that must fail, and a clean TypeScript suite that must pass, then asserts each exit code.
