"""End-to-end tests for the integration-lint-new-arms-trip check: the real entry point, no mocks.
...
SCRIPT = Path(__file__).resolve().parents[2] / "check_integration_lint_new_arms_trip.py"
...
def test_e2e_red_check_passes_when_the_command_fails(capsys):
    ...
    assert run(["check_integration_lint_new_arms_trip.py", "fail", "false"]) == 0
...
def test_e2e_red_check_fails_when_the_command_passes(capsys):
    ...
    assert run(["check_integration_lint_new_arms_trip.py", "fail", "true"]) == 1
    assert "::error::[cli]" in capsys.readouterr().out


def test_e2e_clean_check_passes_when_the_command_passes(capsys):
    assert run(["check_integration_lint_new_arms_trip.py", "pass", "true"]) == 0
    assert "[cli] ok" in capsys.readouterr().out


def test_e2e_clean_check_fails_when_the_command_fails(capsys):
    assert run(["check_integration_lint_new_arms_trip.py", "pass", "false"]) == 1
    assert "::error::[cli]" in capsys.readouterr().out
