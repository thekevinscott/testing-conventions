#!/usr/bin/env python3
"""Assert the `isolation-red` self-test job's published-CLI exit codes (epic #302, #309).

Repo-only: this module exists *only in this repository*. It is NOT shipped in the published
`testing-conventions` tool; it backs the `isolation-red` job in
`.github/workflows/testing-conventions-selftest.yml`, which
drives the published `unit lint --language rust` command over a crate whose unit test
reaches out of its own module and asserts the non-zero exit that fails a consumer's build, then asserts each exit code.
