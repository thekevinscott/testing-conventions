"""End-to-end test for check_isolation_wired: runs the real `__main__` entry via runpy.

Covers `main`, the file read, and the `__main__` guard against real fixture files — a wired one
and a broken one — asserting exit code and printed line each way. The two guard tests pin the
`__name__ == "__main__"` comparison so mutating it (to `is` or `<=`) is caught.
"""
import runpy
import sys
from pathlib import Path

SCRIPT = Path(__file__).resolve().parents[2] / "check_isolation_wired.py"

BROKEN = 'run: unit coverage --language "$LANGUAGE"\n'
...
def test_e2e_passes_on_a_wired_workflow(tmp_path, capsys):
    ...
    wf.write_text('run: unit lint --language "$LANGUAGE" --config "$CONFIG"\n')
    assert run(["prog", str(wf)]) == 0
    assert "invokes `unit lint`" in capsys.readouterr().out
