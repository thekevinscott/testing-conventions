"""End-to-end tests for the mutation-gate check: the real entry point, no mocks.
...
SCRIPT = Path(__file__).resolve().parents[2] / "check_mutation_gate.py"
...
def test_e2e_red_check_passes_when_the_command_fails(capsys):
    ...
    assert run(["check_mutation_gate.py", "fail", "false"]) == 0
