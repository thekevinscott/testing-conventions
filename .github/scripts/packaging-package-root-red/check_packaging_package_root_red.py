#!/usr/bin/env python3
"""Assert the `packaging-package-root-red` self-test job's published-CLI exit codes (epic #302, #309).

Repo-only: this module exists *only in this repository*. It is NOT shipped in the published
`testing-conventions` tool; it backs the `packaging-package-root-red` job in
`.github/workflows/testing-conventions-selftest.yml`, which
drives the published `packaging` command over a per-package `dist/` tarball that ships a
test file and asserts the non-zero exit that fails a consumer's build, then asserts each exit code.
