"""End-to-end tests for the packaging-package-root-red check: the real entry point, no mocks.
...
SCRIPT = Path(__file__).resolve().parents[2] / "check_packaging_package_root_red.py"
...
def test_e2e_red_check_passes_when_the_command_fails(capsys):
    ...
    assert run(["check_packaging_package_root_red.py", "fail", "false"]) == 0
...
def test_e2e_red_check_fails_when_the_command_passes(capsys):
    ...
    assert run(["check_packaging_package_root_red.py", "fail", "true"]) == 1
    assert "::error::[cli]" in capsys.readouterr().out


def test_e2e_clean_check_passes_when_the_command_passes(capsys):
    assert run(["check_packaging_package_root_red.py", "pass", "true"]) == 0
    assert "[cli] ok" in capsys.readouterr().out


def test_e2e_clean_check_fails_when_the_command_fails(capsys):
    assert run(["check_packaging_package_root_red.py", "pass", "false"]) == 1
    assert "::error::[cli]" in capsys.readouterr().out
