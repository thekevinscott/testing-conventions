...
def test_main_passes_when_every_check_meets_its_expectation(command, capsys):
    ...
    assert m.main(["check_packaging_package_root_red.py"]) == 0
    out = capsys.readouterr().out
    assert "::error::" not in out
    assert "[red per-package dist tarball trips the packaging check] ok" in out
    assert command.call_count == 1


def test_main_fails_when_check_0_is_violated(command, capsys):
    command.side_effect = _returns(0)
    assert m.main(["check_packaging_package_root_red.py"]) == 1
    assert "::error::[red per-package dist tarball trips the packaging check]" in capsys.readouterr().out


def test_main_runs_the_hardcoded_commands(command):
    command.side_effect = _returns(1)
    m.main(["check_packaging_package_root_red.py"])
