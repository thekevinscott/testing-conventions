#!/usr/bin/env python3
"""Assert the `packaging-red` self-test job's published-CLI exit codes (epic #302, #309).

Repo-only: this module exists *only in this repository*. It is NOT shipped in the published
`testing-conventions` tool; it backs the `packaging-red` job in
`.github/workflows/testing-conventions-selftest.yml`, which
drives the published `packaging` command over a built wheel that ships a `*_test.py` and
