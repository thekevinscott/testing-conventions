"""End-to-end tests for the packaging-red check: the real entry point, no mocks.
...
SCRIPT = Path(__file__).resolve().parents[2] / "check_packaging_red.py"
...
def test_e2e_red_check_passes_when_the_command_fails(capsys):
    ...
    assert run(["check_packaging_red.py", "fail", "false"]) == 0
