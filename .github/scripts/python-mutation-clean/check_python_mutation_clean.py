#!/usr/bin/env python3
"""Assert the `python-mutation-clean` self-test job's published-CLI exit codes (epic #302, #309).

Repo-only: this module exists *only in this repository*. It is NOT shipped in the published
`testing-conventions` tool; it backs the `python-mutation-clean` job in
`.github/workflows/testing-conventions-selftest.yml`, which
drives the published `unit mutation --language python` command over a mutation-clean
fixture and asserts the zero exit that proves the wheel-shipped adapter resolves (#258), then asserts each exit code.
