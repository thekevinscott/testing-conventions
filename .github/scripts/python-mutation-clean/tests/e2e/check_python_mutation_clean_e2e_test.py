"""End-to-end tests for the python-mutation-clean check: the real entry point, no mocks.
...
SCRIPT = Path(__file__).resolve().parents[2] / "check_python_mutation_clean.py"
...
def test_e2e_red_check_passes_when_the_command_fails(capsys):
    ...
    assert run(["check_python_mutation_clean.py", "fail", "false"]) == 0
