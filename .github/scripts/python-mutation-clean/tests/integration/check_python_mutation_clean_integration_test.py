...
import check_python_mutation_clean as m  # noqa: E402
...
def test_main_passes_when_every_check_meets_its_expectation(command, capsys):
    ...
    assert m.main(["check_python_mutation_clean.py"]) == 0
    out = capsys.readouterr().out
    assert "::error::" not in out
    assert "[clean Python library passes unit mutation] ok" in out
    assert command.call_count == 1


def test_main_fails_when_check_0_is_violated(command, capsys):
    command.side_effect = _returns(1)
    assert m.main(["check_python_mutation_clean.py"]) == 1
    assert "::error::[clean Python library passes unit mutation]" in capsys.readouterr().out


def test_main_runs_the_hardcoded_commands(command):
    command.side_effect = _returns(0)
    m.main(["check_python_mutation_clean.py"])
