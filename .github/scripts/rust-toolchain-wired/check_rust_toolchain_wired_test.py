"""Colocated unit tests for check_rust_toolchain_wired.
...
import check_rust_toolchain_wired as c  # noqa: E402
...
def test_error_names_the_issue_and_input():
    assert "`rust_toolchain` input" in c.ERROR
    assert "#263" in c.ERROR


def test_bare_input_name_alone_is_enough():
    assert c.find_missing_wiring("inputs.rust_toolchain") is None
