"""End-to-end tests for the rust-toolchain-wired helper: real files, no mocks.
...
SCRIPT = Path(__file__).resolve().parents[2] / "check_rust_toolchain_wired.py"
...
def test_passes_on_a_wired_fixture(tmp_path, capsys):
    ...
    good.write_text("if: ${{ inputs.rust_toolchain }}\nuses: dtolnay/rust-toolchain\n")
...
def test_fails_on_a_broken_fixture(tmp_path, capsys):
    bad = tmp_path / "wf.yml"
    bad.write_text("uses: actions/setup-python@v6\n")
