...
- `clean/` — a fully-covered, colocated, mock-free Python suite. Every rule the
  ...
  jobs are skipped while the Python jobs still run.
...
- `no-sources/` — a directory with no language sources at all. Called with the
  ...
  language's jobs and still pass — the dogfooding case of our own
...
- `integration-waiver/` — a clean Python suite whose one integration test trips
  `no-constant-patch`, lifted by a `[[python.exempt]]` entry in a
  *non-default* config path. Run through the reusable workflow, the call passes only
  because the integration-lint job forwards `--config`; the colocated-test and
  coverage jobs pass alongside it.
- `integration-rust/` — `clean/` and `red/` Rust crates for the rust integration-lint
  arm. `clean/` doubles only an external crate (passes); `red/` doubles the
...
- `integration-typescript/` — `clean/` and `red/` TypeScript integration suites for
  the typescript arm. `clean/` mocks only third-party / Node built-ins
  ...
  coverage job).
- `isolation/` — fixtures for the `unit lint` job:
...
- `monorepo/` — a per-package-lockfile monorepo with **no manifest or lockfile at its own
  ...
  blocked by the `@v0` rolling-release lag. The `monorepo-coverage-*` and
  `mutation-monorepo-*` jobs drive this same fixture through real per-package `uses:` calls.
