...
// VIOLATION (`no-first-party-mock`): an integration test must run first-party code
// for real, so mocking a first-party collaborator (a relative import) is forbidden.
vi.mock('../src/ledger', () => ({
  record: vi.fn(),
}));
