...
is coherent. When the file holds one substantial function, the file name is the
subject: the colocated test names what it covers, and a coverage or mutation result
reported against the file is a result about one function.

- **Python:** module-level `def` / `async def`.
- **TypeScript:** top-level `function` declarations, and a top-level `const` / `let` / `var` bound to an arrow or function expression.
- **Rust:** `fn` items at the top level of the file.

Methods (a class's, a Rust `impl` block's), nested functions, and callbacks belong to
their owner and are never counted on their own. Length counts code lines in the body —
blank lines, comments, and a Python docstring are excluded, as is the signature.

**Checked:** deterministic, AST-based. `unit one-function-per-file --language <lang>`
flags every function past the first that exceeds the threshold. `max_lines` is set per
...
suite. Location is checked negatively: `integration lint` scans
`<package root>/tests/integration/` and `<package root>/tests/e2e/` when present
(Rust: the crate root's `tests/`), and flags a test file that lands under `tests/`
outside those tiers (`unknown-tier`). Misplacement fails; a package with no
integration or e2e suite passes.
