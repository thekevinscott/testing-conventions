...
The **exemption-approval** gate — `exemptions --base` ([#229](https://github.com/thekevinscott/testing-conventions/issues/229)) — is the same diff-scoped shape: it fails the PR when the diff **adds or modifies** a `[[<language>.exempt]]` entry, so each new or broadened exemption costs a human greenlight. The greenlight is binary and the agent can't grant it — the check passes only once the `tc:exemption-approved` label is applied by a reviewer who is **not** the PR author (the workflow passes `--approved` to the command only then). The command ships now; wiring its reusable-workflow job is the remaining step (the command-first, workflow-next path `unit mutation` took). See [Configure — adding an exemption is a last resort](./configure#adding-an-exemption-is-a-last-resort-that-needs-a-greenlight).
