...
It is dogfooded through the **shipped reusable workflow** (`dogfood.yml`, `source: internals/detect/src`) like `internals/checks` — every gate, including the coverage floor and diff-scoped mutation. `source` is the inner module dir rather than the package root, and that scoping is forced: pointing it at `internals/detect` runs all three tiers together and fails `unit lint`'s `unmocked-collaborator` rule, which has no concept of test tiers — once a first-party package is declared (any `pyproject.toml`), it flags *every* `*_test.py` under the scanned root that imports the package unmocked, `detect_integration_test.py` and `detect_e2e_test.py` included. (This also explains why `detect.py` silently passed `dogfood-github-helpers.yml`'s isolation check for years despite the same nested layout: `.github/actions` never had a `pyproject.toml`, so the rule's first-party-package lookup found nothing and reported no violations at all — not because the layout satisfied it.) So the colocated `detect_test.py` carries the whole unit tier: it builds real package trees under `tmp_path` and runs `compute_outputs` and its helpers against them, rather than mocking the filesystem. `detect-action.yml` still runs the three-tier suite together.

The package carries a `testing-conventions.toml` holding one line — `one_function_per_file = { max_lines = 19 }`, the lowest threshold the module passes today, with `compute_outputs` the single function above it — and **no exemptions**: the unit tier reaches 100% line-and-branch coverage and kills all 302 whole-tree mutants.

Four mutant classes drove a change to the source rather than a test. `is_workspace_member` repeated `cargo_workspace_root`'s ancestor walk verbatim, so every comparison and loop mutant on the copy had a twin on the original; it now delegates (`cargo_workspace_root(...) is not None`), and the duplicate walk's four survivors went with it. `hermetic`'s `version == ""` admitted `<=` and `is` mutants that no string distinguishes from `==`; `not version` states the same decision with nothing to mutate. `_output_delimiter` truncated its digest to 32 characters, an arbitrary length no caller observes and a number killable only by asserting the hash back, so it uses the full digest. And its collision bump was unreachable from any constructible input — a colliding value has to contain its own hash — so the bump moved to `_free_of(base, lines)`, driven directly with a base that collides.
