...
**`packaging_root`** is the directory the packaging scan is pointed at: `<package_root>/dist` for
Python and TypeScript, `<package_root>/target/package` for Rust, and empty when no
`packaging_build` was derived. `cargo package` writes the crate under `target/package` rather than
`dist/`, so a single hardcoded root covers only two of the three languages. The layout is a
function of the manifest, which makes it detect's to derive rather than a ternary in the job's
`env:`. The scan is pointed at that directory and not at `package_root`, because discovery recurses
— given the package root it would also sweep up every unrelated archive in the tree, including the
...
**The `env:` block is the inputs contract.** The manifest's one step binds five `${{ inputs.<name> }}` expressions to the five environment names `detect.py` reads — `LANGUAGES`, `SCAN_PATH`, `CONFIG`, `CALLER_REPOSITORY`, `VERSION`. A binding wired to the wrong input, or dropped by a rename, hands the script the empty string while reading as wired: `SCAN_PATH` then scans nothing, and `CALLER_REPOSITORY` takes the published path, so this repo's own self-test gates itself with the last release's CLI. `internals/detect`'s e2e suite asserts the table binding-by-binding, plus set equality across all three sides — the inputs the manifest declares, the environment names it binds, and the names the script reads — so an input added to the manifest and a name renamed in the script are both covered the day they land.
...
- **Interpreter.** Stdlib-only `python3` (3.11+, for `tomllib`), invoked directly — `python3 internals/detect/src/detect.py` — with the working directory at the consumer checkout root. The entry script imports its sibling modules under `src/` (the script's own directory leads `sys.path`), so the caller materializes a full checkout of this repo — which the composite-action fetch and willfire's tarball mount both do.
