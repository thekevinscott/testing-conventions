...
- **Inputs.** Five positional arguments carrying the caller's `with:` literals, in the order above; their meanings live in `action.yml` and the module docstring. Each takes its default when the command line stops short of it, so an evaluator that wants only a scan path passes two. For an external caller the fourth is the consumer's repo, so the scan takes the published path — the same branch the consumer's own run takes.
...
A change to any of these — the script's path, the interpreter floor, an argument's position or meaning, the output encoding — breaks the external evaluator. Make it deliberately, and update willfire's recipe in the same motion. The move from five environment variables to five arguments is exactly such a change: an evaluator still exporting `LANGUAGES` / `SCAN_PATH` / `CONFIG` / `CALLER_REPOSITORY` / `VERSION` now gets a default scan of the checkout root on the published path, green and wrong, rather than an error.
