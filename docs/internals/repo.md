...
It lives under `internals/` with the repo's other first-party helper packages. As a real package it is dogfooded through the **shipped reusable workflow** (`dogfood.yml`, `path: internals/checks/src`) — colocated-test, isolation, coverage, integration-lint, and diff-scoped mutation — exactly like `packages/python`. The dogfood caller also runs `unit lint` against `packages/node/src`; `dogfood-node-unit-lint-wired` keeps that caller path present.
