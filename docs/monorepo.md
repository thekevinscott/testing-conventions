...
| `unit-coverage` / `mutation` | source + colocated unit tests | `source` (scanned and run); package root (installed) | Recursive scan of `source` for subjects; toolchain provisioning happens at the package root, and the suite itself runs at `source` — TypeScript's mutation call is the one exception, additionally installing and running its adapter at the package root. `unit-coverage`, in every language, and `mutation` for Python and TypeScript additionally exclude a `tests/` directory directly beneath the root the suite runs at, so the suite tiers stay out of the unit lane |
