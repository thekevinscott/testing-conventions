...
| `unit mutation`       | off (not yet wired)                  | Lands per-language (Rust today, `unit mutation --language rust`); ships to the workflow only once all three reach parity (#199). A binary gate, on by default — see below. |
