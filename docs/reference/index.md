...
Write the managed contract block into the repository's agent context file, so a coding agent
learns the testing contract before it writes code (#232; the
[how-to](../guide/agents) covers the workflow). The block is sentinel-delimited, schema-versioned,
and content-hashed; everything outside the markers is preserved byte for byte.

```
testing-conventions agents install [PATH]
```

| Argument | Description                                              |
| -------- | -------------------------------------------------------- |
| `[PATH]` | The file to manage. Defaults to `AGENTS.md` in the cwd.  |

Upsert semantics, in order:

- **No file** → the file is created containing exactly the block; prints `installed`.
- **File without markers** → the block is appended after the existing content (separated by one
  blank line); prints `installed`.
- **Markers present, region differs from the current template** → only the region between the
  markers is replaced; prints `updated`.
- **Markers present, region already matches** → the file is untouched (byte-identical no-op);
  prints `current`.

Exits `0` in all four cases. Writes are atomic (temp file + rename in the target's directory).
When `PATH` is a symlink the command warns on stderr, writes nothing, and exits `1`.

The begin marker records the block schema version and the first 12 hex chars of the SHA-256 of
the managed region's content:

```md
<!-- testing-conventions:begin v1 hash=<12 hex chars> -->
...
Report whether the managed block is current, without writing anything: the CI side of the
freshness nudge.

```
testing-conventions agents check [PATH]
```

| Argument | Description                                              |
| -------- | -------------------------------------------------------- |
| `[PATH]` | The file to inspect. Defaults to `AGENTS.md` in the cwd. |

Prints exactly one word on stdout and exits accordingly:

| Output    | Exit | Meaning                                                                 |
| --------- | ---- | ----------------------------------------------------------------------- |
| `current` | `0`  | The managed region matches the CLI's current template.                  |
| `stale`   | `1`  | The region differs — the template moved (upgrade) or the block was hand-edited. |
| `absent`  | `1`  | `PATH` does not exist, or it carries no marker pair.                    |

### `agents remove`

Delete the managed block — markers included — leaving the rest of the file byte-for-byte intact.

```
testing-conventions agents remove [PATH]
```
...
Prints `removed` (exit `0`) when a block was deleted, `absent` (exit `0`) when there was no file
or no block — removal is idempotent. Writes are atomic; a symlinked `PATH` is refused with a
warning on stderr and exit `1`, same as `install`.
