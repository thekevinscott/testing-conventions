"""Colocated unit tests for `@path` expansion (isolation — the read is injected).

The read is a dict lookup, so the walk is exercised without a filesystem. Each case asserts on the
expanded text, which is what the budget is measured against.
"""
from checks.agents_md_size.expand import expanded


def _expand(files, entry="AGENTS.md", **kwargs):
    return expanded(entry, files.get, **kwargs)


def test_a_document_with_no_imports_expands_to_itself():
    assert _expand({"AGENTS.md": "solo\n"}) == "solo\n"


def test_a_missing_entry_expands_to_nothing():
    assert _expand({}) == ""


def test_an_empty_entry_expands_to_nothing_without_being_treated_as_missing():
    assert _expand({"AGENTS.md": ""}) == ""


def test_an_imported_file_is_appended_to_the_importer():
    assert _expand({"AGENTS.md": "@a.md\n", "a.md": "aye\n"}) == "@a.md\naye\n"


def test_imports_are_appended_in_document_order():
    files = {"AGENTS.md": "@a.md\n@b.md\n", "a.md": "aye\n", "b.md": "bee\n"}
    assert _expand(files) == "@a.md\n@b.md\naye\nbee\n"


def test_a_missing_import_is_skipped_rather_than_failing():
    assert _expand({"AGENTS.md": "@gone.md\n"}) == "@gone.md\n"
