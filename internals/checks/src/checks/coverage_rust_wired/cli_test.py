"""Colocated unit tests for the coverage-rust-wired check."""
from checks.coverage_rust_wired.cli import REUSABLE_WORKFLOW, cli
...
def test_command_echoes_on_a_wired_workflow(tmp_path, capsys):
    ...
    workflow.write_text("language: ${{ needs.detect.outputs.coverage_languages }}\n")
...
def test_command_raises_on_an_unwired_workflow(tmp_path):
    ...
    try:
        ...
        assert "coverage_languages" in error.message
        assert error.message.endswith(
            "the shipped Rust coverage arm is not enforced on the @v0 path"
        )
    else:
