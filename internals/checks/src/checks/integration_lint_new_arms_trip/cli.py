...
`.github/workflows/testing-conventions-selftest.yml` drives the hermetic-CLI (built-from-HEAD) `integration lint` command over the Rust and TypeScript arms — a violating crate and suite that must fail, and a clean TypeScript suite that must pass — and asserts each exit code (#126 #302 #309).
