"""Colocated unit tests for the rust-toolchain-wired check (isolation — no `CliRunner`).
...
"""
from checks.rust_toolchain_wired.cli import REUSABLE_WORKFLOW, cli


def test_echoes_on_a_wired_workflow(tmp_path, capsys):
    workflow = tmp_path / "wf.yml"
    workflow.write_text("if: ${{ inputs.rust_toolchain }}\n")
    cli.callback(workflow=str(workflow))
    assert "provisions a Rust toolchain behind `rust_toolchain`" in capsys.readouterr().out


def test_raises_on_an_unwired_workflow(tmp_path):
    workflow = tmp_path / "wf.yml"
    workflow.write_text("uses: actions/setup-python@v6\n")
    try:
        cli.callback(workflow=str(workflow))
