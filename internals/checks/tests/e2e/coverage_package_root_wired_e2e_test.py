"""End-to-end tests for the coverage-package-root-wired command: real files, click's CliRunner.
...
from checks.coverage_package_root_wired.cli import cli
...
# The reference is dropped from the unit-coverage job.
UNWIRED = (
    "jobs:\n  unit-coverage:\n    run: npm ci\n"
    "  coverage-changed:\n    run: ${{ needs.detect.outputs.package_root }}\n"
    "  mutation:\n    run: echo done\n"
...
def test_passes_on_a_wired_fixture(tmp_path):
    ...
    assert "install/build at the derived package root" in result.output
