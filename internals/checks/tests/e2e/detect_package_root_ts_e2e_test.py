"""End-to-end tests for the detect-package-root-ts command: click's CliRunner.
...
def test_passes_on_the_expected_outputs():
    ...
    assert "package_root=.github/selftest/monorepo/packages/ts" in result.output
    assert "ts_package_manager=npm" in result.output


def test_fails_on_a_wrong_output():
    bad = list(GOOD_ARGS)
    bad[1] = "pnpm"  # wrong ts_package_manager
    result = CliRunner().invoke(cli, bad)
    assert result.exit_code == 1
    assert "::error::expected ts_package_manager=npm" in result.output
