"""End-to-end tests for the diff-scoped-wired command: real files, click's CliRunner.
...
from checks.diff_scoped_wired.cli import cli

REPO_ROOT = Path(__file__).resolve().parents[4]
WIRED = "run: unit colocated-test --base $BASE\nrun: unit coverage --base $BASE\n"
UNWIRED = "run: unit colocated-test --base $BASE\nrun: unit coverage --language py\n"
...
def test_passes_on_a_wired_fixture(tmp_path):
    ...
    assert "runs the diff-scoped --base checks" in result.output
