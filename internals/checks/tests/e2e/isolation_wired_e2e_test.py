"""End-to-end tests for the isolation-wired command: real files, click's CliRunner.
...
from checks.isolation_wired.cli import cli

REPO_ROOT = Path(__file__).resolve().parents[4]
WIRED = 'jobs:\n  lint:\n    run: unit lint --language "$LANGUAGE"\n'
...
def test_passes_on_a_wired_fixture(tmp_path):
    ...
    assert "invokes `unit lint`" in result.output
