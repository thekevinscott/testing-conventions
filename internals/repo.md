...
The `.github/scripts/` helper scripts are first-party Python and are held to the same conventions as the shipped packages, not waved through: `dogfood-github-helpers.yml` runs `unit colocated-test`, `unit lint`, `unit coverage`, and `integration lint` whole-tree, plus the diff-scoped `unit mutation --base origin/main` (the published binary, via `npx`), over that directory. So `move_major_tag.py` and the not-yet-migrated #302 checks carry a colocated unit test, stay isolated, meet the coverage floor, pass the mocking lint, and kill every mutant on the lines a PR touches — like any package source. Each helper keeps its colocated unit test next to the source and its integration/e2e suites under `tests/` (uniquely named, so `unit coverage`'s pytest collects them without an import-mode flag). `detect.py` (the detect composite action's source) used to be covered here too, scanned as a loose script alongside these; #363 moved it to `internals/detect`, a real uv package dogfooded through the shipped reusable workflow instead (see below) — the same move `internals/checks` made for the #302 checks it's absorbed so far.
...
The static wiring/assertion checks — those confirming the reusable workflow invokes each gate and that the rolling tag advances only through the gated `move-major-tag.yml` — are standalone modules under `.github/scripts/<check>/` too (#304): `mutation-wired`, `isolation-wired`, `coverage-rust-wired`, `colocated-rust-wired`, `diff-scoped-wired`, `e2e-verify-wired`, `e2e-verify-checks-out-pr-head` (block-scoped to the `e2e-verify` job, replacing the old `awk` range), and `rolling-release-wired` (two selftest steps folded into one module). Each holds its inspection in a pure function the colocated unit test drives in isolation; `main` reads the workflow file (repo-relative default path, overridable via `argv`) and prints a `::error::` line on a missing wire.

## The detect action source (`internals/detect`)

`detect.py` — the logic backing the `detect` composite action — moved from `.github/actions/detect/` into `internals/detect/` (#363), leaving `.github/actions/detect/` holding only `action.yml`, the composite-action manifest every consumer's `uses: thekevinscott/testing-conventions/.github/actions/detect@v0` pins. That `uses:` path is an external contract and cannot move; only the Python implementation underneath it did.

`internals/detect/` is a small uv package (`pyproject.toml`, `src/detect.py`, a colocated `src/detect_test.py`, `tests/integration/`, `tests/e2e/`) — the same shape `internals/checks` established, minus the `checks/<name>/` subpackage split, since `detect.py` is one flat module rather than a click-group of per-check commands. It carries no third-party dependencies, so `python_env` derives `pip`; the `pyproject.toml` exists purely so `derive_package_root` finds a manifest and scopes a coverage/mutation venv correctly, exactly as `internals/checks`'s does. Its `[tool.hatch.build] exclude = ["**/*_test.py"]` preempts the same wheel/sdist test-shipping bug #354 found in `internals/checks` — a colocated `detect_test.py` would otherwise ship inside a built distribution if `packaging_build` ever scans this path.

Unlike `internals/checks`, it is **not** dogfooded through the shipped reusable workflow's `unit coverage` gate: that gate measures only the *colocated* unit tier, and `detect.py`'s colocated `detect_test.py` deliberately covers just its pure, filesystem-free helpers — `compute_outputs` and the rest of the orchestration are covered by the integration suite (filesystem mocked) and the e2e suite (a real tree), by original design (predating the move). `internals/checks` reaches 100% from its colocated tier alone only because each check's `cli.py` is a thin wrapper whose colocated test drives `.callback` directly; `detect.py`'s shape doesn't fit that mold, and forcing it through the same gate errors on coverage measured against the wrong tier (verified — the colocated tier alone measures ~20%). Restructuring `detect.py` to fit is out of scope for a pure relocation; `detect-action.yml` remains its coverage/correctness gate instead.

`action.yml`'s `run:` step can no longer find `detect.py` beside itself via `$GITHUB_ACTION_PATH`, so it walks back up three levels (`.github/actions/detect` → the checkout root) and back down into `internals/detect/src/detect.py`:

```bash
python3 "$GITHUB_ACTION_PATH/../../../internals/detect/src/detect.py"
```

That path arithmetic holds whether the action runs locally (`./.github/actions/detect`, this repo's own checkout) or as a consumer's `uses: owner/repo/.github/actions/detect@ref` (GitHub fetches the whole referenced ref into a cache dir, so `internals/detect/` is present alongside `.github/actions/detect/` in that same clone — `$GITHUB_ACTION_PATH` is just the subdirectory containing `action.yml` within it).

`internals/detect` earns two layers of gate, unchanged from before the move beyond their trigger paths and the pytest target:
- **`detect-action.yml`'s `test` job** — a plain `pytest internals/detect/ -v` run (colocated + integration + e2e together, all three tiers combined, no `--cov` floor enforced in CI) — the correctness gate. Verify coverage locally with `pytest internals/detect/ --cov=detect --cov-report=term-missing` (from `internals/detect/`, with `src` on `pythonpath`) — all three tiers combined reach 100%, unlike the colocated tier alone.
- **`detect-action.yml`'s `action` job** — exercises the composite action itself end to end (`uses: ./.github/actions/detect` against a staged fixture, asserting on the real step outputs), the only layer that would catch the `action.yml` → `internals/detect/src/detect.py` wiring breaking.

`dogfood-github-helpers.yml` no longer scans `.github/actions/` at all — nothing but `action.yml` (YAML) is left there to scan.
