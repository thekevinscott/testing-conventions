...
Sub-issue #305 moves the detect-wiring checks across: `wiring-detect-action`, `wiring-packaging-default-on`, and `wiring-e2e-default-on` (static greps over `testing-conventions.yml`), plus `detect-routes-python`, which keeps its `uses: ./.github/actions/detect` step and passes the action's `isolation_languages` output to the script as a CLI argument.
