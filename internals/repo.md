...
- **The engines and runners are third-party, so uv layers them from a hash-pinned lock.** `coverage`, `pytest`, `cosmic-ray`, and `maturin` live nowhere in this repo; a job that needs them runs under `uv run --with-requirements "$GITHUB_WORKSPACE/.github/uv/engines.txt"`, which stands up an ephemeral environment from the committed lock and puts its `python3` first on `PATH` — the same `python3` a spawned adapter or a `coverage`/`pytest` shellout resolves to. `--no-project` is added where the job's directory is not a uv project (`.github/scripts/move-major-tag`, `packages/rust`, `packages/python` for the maturin build); `--project internals/checks` (or `internals/detect`) is used where it is.

  The lock — not a bare `uv run --with coverage` — is load-bearing (#437). A bare `--with pytest` resolves whatever the index serves that day, so a required check reads a mutable external reference that can change without a commit: a new `pytest` can turn a required check red with no commit to blame. That is the class the CI-hermeticity invariant (AGENTS.md, "CI hermeticity") exists to kill, and it holds no less for a third-party engine than for the `@v0` binary — the earlier reasoning that these tools "carry no version-skew risk because they are not the code under test" conflated *correctness* skew (real — latest `pytest` is the `pytest` you want) with *reproducibility* (a required check whose result can move without a commit). `.github/uv/engines.txt` is `uv pip compile`d with `--generate-hashes` from `.github/uv/engines.in`, so it pins every engine transitively and the resolution is a function of the commit under test, exactly as `uv.lock` pins every other uv surface here. `uv pip compile --upgrade .github/uv/engines.in` is the deliberate, diff-visible bump (the version deltas land in the PR); `engines-locked-wired` (`internals/checks`) guards that no workflow floats a `--with <engine>` again. The one thing the float bought — CI exercising the engine a consumer would resolve today — moves to `engines-drift.yml` (#439): a scheduled, non-required job that recompiles `engines.in` at latest and reds when the resolution diverges from the committed lock, naming what moved. A red there blocks nothing; it is the prompt to advance the pin (`uv pip compile --upgrade`) and re-test deliberately, so a breaking engine release is caught on its own rather than tangled into an unrelated bump. The parse-and-compare that decides red is the colocated-tested `tc-checks engines-drift`; the workflow's `uv pip compile --upgrade` is a plain tool invocation, so the job holds no untested YAML logic.
...
The engine lock (#437) is repo-internal — it pins the engines *this* repo's own jobs layer. The **reusable workflow** provisions a **consumer's** suite toolchain (`uv pip install coverage pytest testing-conventions`), and that runs in the consumer's checkout, where `.github/uv/engines.txt` does not exist: a `run:` step shipped at `@v0` cannot `--with-requirements` a file only this repo holds. So the consumer-facing pin travels the only way it can — written inline into the workflow YAML: `uv pip install "coverage==<v>" "pytest==<v>" testing-conventions` (#438), keeping a consumer's required check on a fixed `coverage`/`pytest` rather than index-latest. The named engines are pinned; their transitive deps resolve compatibly under the pin (full transitive pinning would need a lock file, which can't travel to a consumer). `testing-conventions` on that line stays unpinned — it is the package under test, resolved by the published/HEAD-wheel rules above, not a third-party engine. `suite-toolchain-pinned-wired` guards the inline pin; the inline versions mirror `engines.txt` by convention.
