...
- **The engines and runners are third-party, so uv layers them from an index.** `coverage`, `pytest`, `cosmic-ray`, and `maturin` live nowhere in this repo; a job that needs them runs under `uv run --with coverage --with pytest --with cosmic-ray` (or `--with maturin`), which stands up an ephemeral environment and puts its `python3` first on `PATH` — the same `python3` a spawned adapter or a `coverage`/`pytest` shellout resolves to. `--no-project` is added where the job's directory is not a uv project (`.github/scripts/move-major-tag`, `packages/rust`, `packages/python` for the maturin build); `--project internals/checks` (or `internals/detect`) is used where it is. These tools carry no version-skew risk: they are third-party dependencies, not the code under test, so the release the index resolves is the release the check wants.
...
The **reusable workflow** provisions a **consumer's** suite toolchain (`uv pip install coverage pytest testing-conventions`), and that line runs in the consumer's required check — a floating `coverage`/`pytest` there reds a consumer's build on a new release with no commit to blame. So it is pinned inline: `uv pip install "coverage==<v>" "pytest==<v>" testing-conventions` (#438). Inline is the only pin that reaches a consumer — the workflow ships at `@v0` into a checkout that holds none of this repo's own packages or locks, so a `run:` step there can't reference a lock file only this repo carries. The named engines are pinned; their transitive deps resolve compatibly under the pin (full transitive pinning would need a lock file, which can't travel to a consumer). `testing-conventions` on that line stays unpinned — it is the package under test, resolved by the published/HEAD-wheel rules above, not a third-party engine. `suite-toolchain-pinned-wired` guards the inline pin. (Pinning the repo's *own* internal engines — where a package `uv.lock` can hold them — is the rest of epic #436.)
