...
Adds the `exemptions` command (#229) — the exemption-approval gate. `testing-conventions exemptions --base <REF> [--approved]` diffs the `[[<language>.exempt]]` entries between `<REF>` and the working tree's config and exits `1` when the diff **adds or modifies** one, unless `--approved` is passed. Identity is the whole entry (path + rules + reason), so adding, lifting an extra rule, widening scope, or rewording a reason all gate; removing or leaving an entry unchanged is free. `--approved` is the binary human greenlight (the reusable workflow will set it only when a non-author reviewer applied the `tc:exemption-approved` label, so an agent can't approve its own exemption). Purely additive — a new `exemptions` subcommand and `exemptions` module (`changed`, `ChangedExemption`); nothing existing changes. The reusable-workflow job that reads the label is the remaining wiring step, command-first like `unit mutation` (#201–#203 → #204).
